...
# Daily usage reset hour (BDT). Before 04:00 counts as previous day.
USAGE_RESET_HOUR = 4

def _usage_period_id():
    """Return period id for daily limit. Resets every day at 04:00 BDT."""
    now = now_bdt()
    if now.hour < USAGE_RESET_HOUR:
        now = now - timedelta(days=1)
    return now.strftime("%Y-%m-%d")

def _apply_daily_usage_reset(entry):
    """If period changed, zero used counter. Mutates entry dict."""
    if not isinstance(entry, dict):
        return entry
    period = _usage_period_id()
    if entry.get("usage_period") != period:
        entry["used"] = 0
        entry["usage_period"] = period
    return entry
...
def is_valid_key(api_key):
    ...
    try:
        ...
        entry = _apply_daily_usage_reset(entry)
        # persist reset if period flipped
        if allowed[api_key] != entry:
            with config_lock:
                cfg2 = _read_config()
                if api_key in cfg2.get("ALLOWED_KEYS", {}):
                    e2 = _normalize_key_entry(cfg2["ALLOWED_KEYS"][api_key])
                    e2 = _apply_daily_usage_reset(e2)
                    cfg2["ALLOWED_KEYS"][api_key] = e2
                    _write_config(cfg2)
                    entry = e2
...
def _key_response_fields(key):
    if _auth_matches_admin(key):
        return {"KeyExpiresAt": "unlimited", "KeyRemainingRequests": "∞/∞",
                "UsageResetsAt": "N/A", "UsagePeriod": "admin"}
    try:
        cfg = _read_config()
        ...
        if info:
            info = _apply_daily_usage_reset(dict(info))
    except Exception: info = None
    if not info: return {"KeyExpiresAt": "N/A", "KeyRemainingRequests": "0/0",
                         "UsageResetsAt": "N/A", "UsagePeriod": "N/A"}
    try:
    ...
    # next reset = next 04:00 BDT
    now = now_bdt()
    reset_today = now.replace(hour=USAGE_RESET_HOUR, minute=0, second=0, microsecond=0)
    if now >= reset_today:
        next_reset = reset_today + timedelta(days=1)
    else:
        next_reset = reset_today
    return {"KeyExpiresAt": ea, "KeyRemainingRequests": rem,
            "UsageResetsAt": next_reset.isoformat(),
            "UsagePeriod": info.get("usage_period") or _usage_period_id()}
...
@app.get("/api")
def api_catalog():
    """Client APIs — only /like needs API key; everything else is free."""
    base = (request.url_root or "").rstrip("/")
    apis = [
        {"method": "GET", "path": "/like", "auth": "key", "desc": "Send likes (API key required)",
         "params": ["uid", "server_name", "key"],
         "example": f"{base}/like?uid=UID&server_name=BD&key=YOUR_KEY"},
        {"method": "GET/POST", "path": "/bio", "auth": False, "desc": "Update bio/signature — FREE",
         "params": ["uid", "password", "bio"]},
        {"method": "GET/POST", "path": "/friend/add", "auth": False, "desc": "Add friend — FREE",
         "params": ["uid", "password", "friend_uid", "server_name"]},
        {"method": "GET/POST", "path": "/friend/remove", "auth": False, "desc": "Remove friend — FREE",
         "params": ["uid", "password", "friend_uid", "server_name"]},
        {"method": "GET/POST", "path": "/friend/list", "auth": False, "desc": "Friend list — FREE",
         "params": ["uid", "password", "server_name"]},
        {"method": "GET/POST", "path": "/guild/join", "auth": False, "desc": "Join guild — FREE",
         "params": ["uid", "password", "clan_id", "server_name"]},
        {"method": "GET/POST", "path": "/guild/leave", "auth": False, "desc": "Leave guild — FREE",
         "params": ["uid", "password", "server_name"]},
        {"method": "GET/POST", "path": "/eat", "auth": False, "desc": "EAT / convert guest — FREE",
         "params": ["uid", "password"]},
        {"method": "GET/POST", "path": "/token", "auth": False, "desc": "JWT from uid+password — FREE",
         "params": ["uid", "password"]},
        {"method": "GET/POST", "path": "/access", "auth": False, "desc": "JWT from access_token — FREE",
         "params": ["access_token"]},
        {"method": "GET", "path": "/info", "auth": False, "desc": "Full player info — FREE",
         "params": ["uid"], "example": f"{base}/info?uid=UID"},
        {"method": "GET", "path": "/region", "auth": False, "desc": "Player region / likes — FREE",
         "params": ["uid"]},
        {"method": "GET", "path": "/level", "auth": False, "desc": "Level + EXP — FREE",
         "params": ["uid"]},
        {"method": "GET", "path": "/check", "auth": False, "desc": "Ban check — FREE",
         "params": ["uid"]},
        {"method": "GET", "path": "/health", "auth": False, "desc": "Health check — FREE"},
        {"method": "GET", "path": "/stats", "auth": False, "desc": "Public stats — FREE"},
        {"method": "GET", "path": "/settings", "auth": False, "desc": "Public branding — FREE"},
        {"method": "GET", "path": "/api", "auth": False, "desc": "This catalog — FREE"},
    ]
    for a in apis:
        a["url"] = base + a["path"]
    return _jsonify({
        ...
        "service": BRAND_NAME,
        "now_bdt": now_bdt_str(),
        "note": "Only /like requires API key. All other endpoints are free.",
        "usage_reset": {
            "hour_bdt": USAGE_RESET_HOUR,
            "timezone": "Asia/Dhaka (BDT)",
            "note": f"/like key daily limit resets at {USAGE_RESET_HOUR:02d}:00 BDT",
            "current_period": _usage_period_id(),
        },
        "auth_notes": {
            "like": "GET /like?uid=&server_name=&key=YOUR_KEY",
            "free": "bio, friend, guild, eat, token, access, info, region, level, check — no key",
        },
        "total": len(apis),
        "apis": apis,
    })
...
if __name__ == "__main__":
    ...
    print(f"  Usage reset: daily {USAGE_RESET_HOUR:02d}:00 BDT")
    print("=" * 60)
