# main.py — SB AURA Backend — FULL (All APIs: detailed main responses)

import os, json, time, binascii, base64, asyncio, urllib3, tempfile
...
import requests, httpx, aiohttp, jwt
...
def is_valid_key(api_key):
    if api_key == MASTER_ACCESS_KEY: return True
    try:
        cfg = _read_config()
...
def increment_key_usage(api_key):
    if api_key == MASTER_ACCESS_KEY: return
    try:
        cfg = _read_config()
...
def generate_jwts_live(accounts, max_workers=JWT_WORKERS):
    ...
    total = len(accounts); tokens = []; failed_accounts = []; t0 = time.time()
    ...
    _log(f"✅ First pass: {len(tokens)}/{total}", "ok")
    if failed_accounts:
        _log(f"↻ Retrying {len(failed_accounts)} failed...", "warn")
        second_batch = []; rw = max(1, min(max_workers // 2, 30))
    ...
    _log(f"🎯 Done: {len(tokens)}/{total} in {round(time.time()-t0,1)}s", "ok")
    return tokens
...
def _build_token_cache(srv):
    _log(f"🔄 Building cache for {srv}...", "info"); t0 = time.time()
    accounts = _load_accounts_for_server(srv)
    if not accounts:
        _log(f"❌ No accounts for {srv}", "err")
        with TOKEN_CACHE_LOCK:
            TOKEN_CACHE[srv] = {"tokens": [], "expiry": 0, "building": False,
                                "generated_at": "", "total_accounts": 0}
        return []
    tokens = generate_jwts_live(accounts, max_workers=JWT_WORKERS)
    expiry = time.time() + TOKEN_TTL
    with TOKEN_CACHE_LOCK:
        TOKEN_CACHE[srv] = {"tokens": tokens, "expiry": expiry, "building": False,
                            "generated_at": now_bdt_str(), "total_accounts": len(accounts)}
    _save_token_cache_to_disk(srv, tokens, expiry)
    _log(f"💾 Cache saved: {srv} — {len(tokens)}/{len(accounts)}", "ok")
    return tokens
...
def refresh_token_cache_background(srv):
    def _w():
        ...
        except Exception as e: _log(f"❌ BG refresh {srv}: {e}", "err")
    _threading.Thread(target=_w, daemon=True).start()

def _token_cache_scheduler():
    _log("⏰ Token scheduler started (5h50m)", "info")
    while True:
        try:
            time.sleep(5 * 3600 + 50 * 60)
            for srv in list(SERVER_ACCOUNT_FILES.keys()):
                try:
                    if _load_accounts_for_server(srv): refresh_token_cache_background(srv)
                except Exception as e: _log(f"⏰ Sched err {srv}: {e}", "err")
        except Exception as e: _log(f"⏰ Sched err: {e}", "err")

_threading.Thread(target=_token_cache_scheduler, daemon=True).start()
...
# ═══════════════════════════════════════════════════════════
#  LOGIN RESOLVER (returns full details)
# ═══════════════════════════════════════════════════════════
def _get_login_jwt_info(uid=None, password=None, access_token=None, jwt_token=None):
    """Returns (jwt, method, uid, name, region, open_id, access_token, error)."""
    if jwt_token:
...
# ═══════════════════════════════════════════════════════════
#  FRIEND ADD / REMOVE / LIST — FULL RESPONSES
# ═══════════════════════════════════════════════════════════
...
def _friend_remove(jwt_token, author_uid, target_uid, region):
    ...
    if not _HAS_REMOVE_FRIEND:
        base["message"] = "RemoveFriend_Req_pb2.py missing"
        return base
    try:
        player = _get_target_player_info(target_uid, jwt_token, region)
...
# ═══════════════════════════════════════════════════════════
#  GUILD JOIN / LEAVE — FULL RESPONSES
# ═══════════════════════════════════════════════════════════
def _guild_join(jwt_token, clan_id, region, uid, name):
    ...
    if not _HAS_REQ_CLAN:
        base["error"] = "ReqCLan_pb2.py missing"
        return base
    try:
...
def _guild_leave(jwt_token, clan_id, region, uid, name):
    ...
    if not _HAS_QUIT_CLAN:
        base["error"] = "QuitClanReq_pb2.py missing"
        return base
    try:
...
@app.get("/")
def index():
    return render_template("index.html", brand=BRAND_NAME, dev=DEV_NAME,
                           tg=TELEGRAM, owner=OWNER_HANDLE, badge="Premium")
...
# ═══════════════════════════════════════════════════════════
#  /like — SHORT RESPONSE (as you requested)
# ═══════════════════════════════════════════════════════════
@app.get("/like")
def handle_like():
    try:
...
def bio_endpoint():
    try:
        ...
        _log(f"📝 /bio UID={f_uid} uploading via {method}", "info")
        ...
        return _jsonify({
            ...
            "code": result["code"],
            "bio": bio,
            "uid": str(f_uid) if f_uid else None,
            "name": f_name,
            "region": f_region,
            "login_method": method,
            "open_id": f_oid,
...
def friend_add_endpoint():
    try:
        ...
        result["open_id"] = f_oid
        result["access_token"] = f_at
...
if __name__ == "__main__":
    ...
    print(f"  Master key: {MASTER_ACCESS_KEY}")
    print("=" * 60)
